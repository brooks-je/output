...
class Geobam_wse(AbstractModule):
    """
    A class that represents the results of running geobam_wse.

    Data and operations append geobam_wse results to the SoS on the appropriate
    dimensions.
    ...
    def get_module_data(self):
        ...
        nb_dir = os.path.join(self.input_dir, 'geobam_wse')
        ...
        if len(nb_files) != 0:
            ...
            for s_rid in self.sos_rids:
                if s_rid in nb_rids:
                    try:
                        nb_ds = Dataset(os.path.join(nb_dir , f"{int(s_rid)}_geobam_wse.nc"), 'r')
                        ...
                        #  r, logn, logWb, logDb, f, logDc, z0, q 
                        ...
                        for a_node_id in abs(nb_ds.node_ids)[:]:
                            ...
                            nb_dict["f"]["mean"][node_index] = nb_ds["f"]["mean"][internal_node_count].filled(np.nan)
                            nb_dict["f"]["attrs"]['mean']['_FillValue'] = nb_ds["f"]['mean']._FillValue
                            nb_dict["f"]["sd"][index] = nb_ds["f"]["sd"][0].filled(np.nan)
                            nb_dict["f"]["attrs"]['sd']['_FillValue'] = nb_ds["f"]['sd']._FillValue

                            nb_dict["logDc"]["mean"][node_index] = nb_ds["logDc"]["mean"][internal_node_count].filled(np.nan)
                            nb_dict["logDc"]["attrs"]['mean']['_FillValue'] = nb_ds["logDc"]['mean']._FillValue
                            nb_dict["logDc"]["sd"][index] = nb_ds["logDc"]["sd"][0].filled(np.nan)
                            nb_dict["logDc"]["attrs"]['sd']['_FillValue'] = nb_ds["logDc"]['sd']._FillValue

                            nb_dict["z0"]["mean"][node_index] = nb_ds["z0"]["mean"][internal_node_count].filled(np.nan)
                            nb_dict["z0"]["attrs"]['mean']['_FillValue'] = nb_ds["z0"]['mean']._FillValue
                            nb_dict["z0"]["sd"][index] = nb_ds["z0"]["sd"][0].filled(np.nan)
                            nb_dict["z0"]["attrs"]['sd']['_FillValue'] = nb_ds["z0"]['sd']._FillValue

                            internal_node_count += 1

                        nb_ds.close()
                    except:
                        self.logger.warn(f'Reach {s_rid} failed for geobam_wse...')
                index += 1
    ...
    def append_module_data(self, data_dict, metadata_json):
        ...
        nb_grp = sos_ds.createGroup("geobam_wse")
        
        r_grp = nb_grp.createGroup("r")
        var = self.write_var(r_grp, "mean", "f8", ("num_nodes"), data_dict["r"])
        self.set_variable_atts(var, metadata_json["geobam_wse"]["r"]["mean"])
        var = self.write_var(r_grp, "sd", "f8", ("num_reaches"), data_dict["r"])
        self.set_variable_atts(var, metadata_json["geobam_wse"]["r"]["sd"])
        
        logn_grp = nb_grp.createGroup("logn") 
        var = self.write_var(logn_grp, "mean", "f8", ("num_nodes"), data_dict["logn"])
        self.set_variable_atts(var, metadata_json["geobam_wse"]["logn"]["mean"])
        var = self.write_var(logn_grp, "sd", "f8", ("num_reaches"), data_dict["logn"])
        self.set_variable_atts(var, metadata_json["geobam_wse"]["logn"]["sd"])
        
        logDb_grp = nb_grp.createGroup("logDb") 
        var = self.write_var(logDb_grp, "mean", "f8", ("num_nodes"), data_dict["logDb"])
        self.set_variable_atts(var, metadata_json["geobam_wse"]["logDb"]["mean"])
        var = self.write_var(logDb_grp, "sd", "f8", ("num_reaches"), data_dict["logDb"])
        self.set_variable_atts(var, metadata_json["geobam_wse"]["logDb"]["sd"])

        f_grp = nb_grp.createGroup("f") 
        var = self.write_var(f_grp, "mean", "f8", ("num_nodes"), data_dict["f"])
        self.set_variable_atts(var, metadata_json["geobam_wse"]["f"]["mean"])
        var = self.write_var(f_grp, "sd", "f8", ("num_reaches"), data_dict["f"])
        self.set_variable_atts(var, metadata_json["geobam_wse"]["f"]["sd"])

       logDc_grp = nb_grp.createGroup("logDc") 
        var = self.write_var(logDc_grp, "mean", "f8", ("num_nodes"), data_dict["logDc"])
        self.set_variable_atts(var, metadata_json["geobam_wse"]["logDc"]["mean"])
        var = self.write_var(logDc_grp, "sd", "f8", ("num_reaches"), data_dict["logDc"])
        self.set_variable_atts(var, metadata_json["geobam_wse"]["logDc"]["sd"])

        z0_grp = nb_grp.createGroup("z0") 
        var = self.write_var(z0_grp, "mean", "f8", ("num_nodes"), data_dict["z0"])
        self.set_variable_atts(var, metadata_json["geobam_wse"]["z0"]["mean"])
        var = self.write_var(z0_grp, "sd", "f8", ("num_reaches"), data_dict["z0"])
        self.set_variable_atts(var, metadata_json["geobam_wse"]["z0"]["sd"])
        
        logWb_grp = nb_grp.createGroup("logWb") 
        var = self.write_var(logWb_grp, "mean", "f8", ("num_nodes"), data_dict["logWb"])
        self.set_variable_atts(var, metadata_json["geobam_wse"]["logWb"]["mean"])
        var = self.write_var(logWb_grp, "sd", "f8", ("num_reaches"), data_dict["logWb"])
        self.set_variable_atts(var, metadata_json["geobam_wse"]["logWb"]["sd"])
        
        q_grp = nb_grp.createGroup("q")
        var = self.write_var_nt(q_grp, "q", self.vlen_f, ("num_reaches"), data_dict["q"])
        self.set_variable_atts(var, metadata_json["geobam_wse"]["q"]["q"])
        var = self.write_var(q_grp, "q_sd", "f8", ("num_reaches"), data_dict["q"])
        self.set_variable_atts(var, metadata_json["geobam_wse"]["q"]["q_sd"])
